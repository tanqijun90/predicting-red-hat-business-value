...
def prediction_mod_funct(prediction,center,confidence_wo):
    ...
    set so that it combines better with the predictions of activities whose
    group are.'''
    return confidence_wo*(prediction-center)+center

def interpolate_funct(x,x_0,x_1,y_0,y_1,center=0.5,alpha=0.5):
    '''Interpolate the group success rate w.r.t. activity date. Note that alpha
    does not change gp_AUC.'''
    l=2*(x-x_0)/(x_1-x_0)-1 #l(x_1)=1,l(x_0)=-1
    phi=np.tanh(alpha*l)/np.tanh(alpha)#phi(1)=1,phi(-1)=-1
    return (y_1-y_0)*phi/2+(y_1+y_0)/2

def interpolate_funct_boundary(
        x, x_0, y_0, boundary, decay, alpha=0.00001, alpha_limit=1):
    '''Predict the group success rate when the date is outside the range of
        dates where the rate is known.'''
    boundary_limit=((y_0+decay*alpha_limit*boundary)/
                    (1+decay*alpha_limit*boundary))
    if y_0==1:
        boundary_limit=0.95
    else:
        boundary_limit=0.05
...
def interpolate_rt(
        df_sorted_by_date,col_of_date_int,col_of_average,col_of_boundary,
        col_of_decay,interpolate_boundary,date_int,fillna_num=60000):
    '''Given group success rate of one group sorted by date and a date,
        interpolate the group success rate at that given date'''
    if interpolate_boundary:
        if date_int > df_sorted_by_date[col_of_date_int].iloc[-1]:
            return interpolate_funct_boundary(
                date_int,df_sorted_by_date[col_of_date_int].iloc[-1],
                df_sorted_by_date[col_of_average].iloc[-1],
                df_sorted_by_date[col_of_boundary].iloc[-1],
                df_sorted_by_date[col_of_decay].iloc[-1])
        if date_int < df_sorted_by_date[col_of_date_int].iloc[0]:
            return (interpolate_funct_boundary(
                date_int,df_sorted_by_date[col_of_date_int].iloc[0],
                df_sorted_by_date[col_of_average].iloc[0],
                df_sorted_by_date[col_of_boundary].iloc[0],
                df_sorted_by_date[col_of_decay].iloc[0]))
        return fillna_num
    else:
        if date_int > df_sorted_by_date[col_of_date_int].iloc[-1]:
            return fillna_num
        ...
        return interpolate_funct(
            date_int,df_sorted_by_date[col_of_date_int].iloc[ind-1],
            df_sorted_by_date[col_of_date_int].iloc[ind],
            df_sorted_by_date[col_of_average].iloc[ind-1],
            df_sorted_by_date[col_of_average].iloc[ind])

def interpolate_rt_wrapped(
        df_to_interpolate,gpb_sorted,col_of_date_int,col_of_average,
        col_of_boundary,col_of_decay,interpolate_boundary):
    '''This is an auxillary function that makes it possible to call
        interpolate_rt via the map function.'''
    rt=df_to_interpolate.apply(
        lambda entry : interpolate_rt(gpb_sorted.loc[entry[0]],
                                      col_of_date_int,
                                      col_of_average,
                                      col_of_boundary,
                                      col_of_decay,
...
def interpolate_rt_df(
        df_to_interpolate,df_data,group_by,col_of_date_int,
        col_of_average,col_of_boundary,col_of_decay,interpolate_boundary=False):
    '''Given a training data and a data to interpolate, for each group_by
        category, the function interpolates the average w.r.t. date.'''
    from functools import partial
    gp_intersect=np.intersect1d(
        df_to_interpolate[group_by].values,df_data[group_by].values)
    df_to_interpolate_gp\
        = df_to_interpolate[df_to_interpolate[group_by].isin(gp_intersect)]
    df_to_interpolate_gp=df_to_interpolate_gp[[group_by,col_of_date_int]]
    gpb=df_data.groupby(group_by)
    gpb_sorted\
        =gpb[[
            col_of_date_int,col_of_average,col_of_boundary,col_of_decay]].apply(
                partial(pd.DataFrame.sort_values,by=col_of_date_int))
    df_to_interpolate_gp.drop_duplicates(inplace=True)
    with Pool(6) as p:
        df_to_interpolate_gp_split=np.array_split(df_to_interpolate_gp,6)
        df_to_interpolate_gp[col_of_average]=pd.concat(
            p.map(partial(interpolate_rt_wrapped,gpb_sorted=gpb_sorted,
                          col_of_date_int=col_of_date_int,
                          col_of_average=col_of_average,
                          col_of_boundary=col_of_boundary,
                          col_of_decay=col_of_decay,
                          interpolate_boundary=interpolate_boundary),
                  df_to_interpolate_gp_split))
    if interpolate_boundary:
        df_to_interpolate_gp.rename(
            columns={col_of_average:col_of_average+'_boundary'},inplace=True)
    ...
    df_to_interpolate=df_to_interpolate.merge(
        df_to_interpolate_gp,how='left',on=[group_by,col_of_date_int])
    # Since we are not just using df_to_interpolate_gp
    df_to_interpolate[col_of_average].fillna(
        fillna_num,inplace=True)
...
def success_rate(data_total,cols_list,data_use=[],fillna_num=60000):
    ...
    data_total=data_total.merge(
        pd.DataFrame(cols_rt),how='left',left_on=cols_list,right_index=True)
...
def unmask_rt(data_total,col_rt,col_over=[],fillna_num=60000):
    ...
    data_total[col_rt+'_s']=((data_total[col_rt]>=0.5) &
                             (data_total[col_rt]<fillna_num))
    data_total[col_rt+'_us']=data_total[col_rt]<0.5
    for col in col_over:
        data_total[col_rt+'_s']=data_total[col_rt+'_s']&data_total[col].isnull()
        data_total[col_rt+'_us']=(data_total[col_rt+'_us'] &
                                  data_total[col].isnull())
...
def prod_feature(data,col1,col2,new_col_name='no_name_provided'):
    ...
    data=data.merge(pd.DataFrame(new_index),how='left',
                    left_on=[col1,col2],right_index=True)
...
def final_data_treat(data,fillna_num=60000):
    for col in data.columns:
        if (data[col].dtype=='bool'):
            data[col]=data[col].astype('int8')
    for col in (set(
            ['char_1_x', 'char_10_x', 'char_2_x','char_3_x',
             'char_4_x', 'char_5_x', 'char_6_x', 'char_7_x',
             'char_8_x','char_9_x']) & set(data.columns)):
...
def hist_bin(data,col,bin_size):
    ...
    hist=((hist_b.cumsum()-hist_b/2)/hist_b.sum()*bin_size).apply(
        np.ceil).astype('int32')
    hist.name=col+'_bin'
    print('{}{}'.format('Actual bin number: ',len(np.unique(hist.values))))
    return data.merge(
        pd.DataFrame(hist),how='left',left_on=col,right_index=True)

def congregate_small_category(data,col,max_num_corr):
    '''Congregate categories with appearances less than max_num_corr into
    categories by the number of appearances.'''
...
# activities. This will be a good mimic of error rate, and will improve our
# auc.
...
m_total=m_total.merge(
    pd.DataFrame(peo_last_act),how='left',
    left_on=['people_id'],right_index=True)
...
m_total['act_week_num']\
    =np.floor(((m_total['date_x']-pd.datetime(2020,1,7)).dt.days)/7)
...
m_total=m_total.merge(
    pd.DataFrame(t_group_act_size),how='left',
...
#We create a m_small to exclude the larges four groups in order to reduce the
#bias they introduce.
...
m_total=success_rate(
    m_total,['char_2_y','char_6_y','char_7_y','char_9_y'],m_train)
...
m_test=interpolate_rt_df(
    m_test,m_train,'group_1','act_date_int',
    'act_date_int_group_1_rt','group_1_rt','group_act_size')
del m_total['act_date_int_group_1_rt']
m_total['act_date_int_group_1_rt']=pd.concat(
    [m_train['act_date_int_group_1_rt'],
     m_test['act_date_int_group_1_rt']]).reset_index(drop=True)

m_test=interpolate_rt_df(
    m_test,m_train,'group_1','act_date_int',
    'act_date_int_group_1_rt','group_1_rt','group_act_size',True)
m_total['act_date_int_group_1_rt_boundary']=pd.concat(
    [m_train['act_date_int_group_1_rt'],
     m_test['act_date_int_group_1_rt_boundary']]).reset_index(drop=True)
del m_train
del m_test
######################################
...
m_total=unmask_rt(m_total,'act_date_int_group_1_rt_boundary',
                  ['act_date_int_group_1_rt'])
m_total=unmask_rt(m_total,'char_2_y_char_6_y_char_7_y_char_9_y_rt',
                  ['act_date_int_group_1_rt'])
# ######################################
...
available_col=['activity_category', 'activity_id', 'char_1_x', 'char_10_x',
    ...
    'char_8_x','char_9_x', 'outcome', 'people_id', 'char_1_y', 'group_1',
...
categorical_col=['activity_category', 'char_1_x', 'char_10_x',
    'char_2_x','char_3_x', 'char_4_x', 'char_5_x', 'char_6_x', 'char_7_x',
    'char_8_x','char_9_x', 'char_1_y', 'group_1', 'char_2_y','char_3_y',
    'char_4_y', 'char_5_y', 'char_6_y', 'char_7_y', 'char_8_y','char_9_y']
use_col_gp=['activity_category', 'char_1_x', 'char_10_x', 'char_2_x','char_3_x',
    ...
    'char_13', 'char_14','char_15', 'char_16', 'char_17', 'char_18', 'char_19',
    'char_20','char_21', 'char_22', 'char_23', 'char_24', 'char_25',
    'char_26','char_27', 'char_28', 'char_29', 'char_30', 'char_31',
    'char_32','char_33', 'char_34', 'char_35', 'char_36', 'char_37', 'char_38']
use_col_wo=['activity_category', 'char_1_x', 'char_2_x','char_3_x', 'char_4_x',
    'char_5_x', 'char_6_x', 'char_7_x', 'char_8_x','char_9_x', 'char_2_y',
...
available_col=available_col+['group_1_con', 'is_last_act', 'act_year',
                             'act_month', 'act_day',
                             'act_weekday','is_weekend','act_week_num',
                             'act_date_int', 'peo_date_int',
                             'group_act_size','char_6_y_prod_char_2_y',
                             'char_5_y_prod_char_6_y','char_7_y_prod_char_9_y',
                             'char_1_y_prod_char_8_y',
                             'char_2_y_char_6_y_char_7_y_char_9_y_rt',
                             'group_1_rt','act_date_int_group_1_rt',
                             'act_date_int_group_1_rt_boundary','group_1_rt_s',
                             'group_1_rt_us',
                             'act_date_int_group_1_rt_boundary_s',
                             'act_date_int_group_1_rt_boundary_us',
                             'char_2_y_char_6_y_char_7_y_char_9_y_rt_s',
                             'char_2_y_char_6_y_char_7_y_char_9_y_rt_us',
                             'fill05']

categorical_col=categorical_col+['group_1_con','act_weekday','act_week_num',
                                 'char_6_y_prod_char_2_y',
                                 'char_5_y_prod_char_6_y',
                                 'char_7_y_prod_char_9_y',
                                 'char_1_y_prod_char_8_y']#'act_year',
#'act_month', 'act_day',

use_col_gp=use_col_gp+['is_last_act', 'act_year', 'act_month', 'act_day',
                       'act_weekday','act_week_num',
                       'is_weekend','act_date_int', 'peo_date_int',
                       'group_act_size','char_6_y_prod_char_2_y',
                       'char_2_y_char_6_y_char_7_y_char_9_y_rt',
                       'group_1_rt','act_date_int_group_1_rt',
                       'act_date_int_group_1_rt_boundary','group_1_rt_s',
...
use_col_wo=use_col_wo+['group_1_con', 'is_last_act', 'act_year', 'act_month',
                       'act_day', 'act_weekday','is_weekend', 'act_date_int',
                       'peo_date_int']#,'group_act_size']#,
#'char_5_y_prod_char_6_y']#,'char_7_y_prod_char_9_y']#,
...
df_train_gp.rename(
    columns={'act_date_int_group_1_rt':'act_date_int_group_1_rt_optimistic'},
    inplace=True)
df_train_gp.rename(columns={'group_1_rt':'group_1_rt_optimistic'},inplace=True)
df_train_gp.rename(
    columns={
        'act_date_int_group_1_rt_boundary':
        'act_date_int_group_1_rt_boundary_optimistic'},
    inplace=True)
df_train_gp.rename(columns={
    'char_2_y_char_6_y_char_7_y_char_9_y_rt':
    'char_2_y_char_6_y_char_7_y_char_9_y_rt_optimistic'},
                   inplace=True)
...
df_train_gp=success_rate(
    df_train_gp,['act_date_int','group_1'],t_df_train_gp_frac)
df_train_gp=success_rate(
    df_train_gp,
    ['char_2_y','char_6_y','char_7_y','char_9_y'],t_df_train_gp_frac)
df_train_gp=success_rate(df_train_gp,['group_1'],t_df_train_gp_frac)

t_df_train_gp_frac=df_train_gp.loc[mock_miss_act]
df_train_gp=interpolate_rt_df(
    df_train_gp,t_df_train_gp_frac,'group_1','act_date_int',
    'act_date_int_group_1_rt','group_1_rt','group_act_size')
df_train_gp=interpolate_rt_df(
    df_train_gp,t_df_train_gp_frac,'group_1','act_date_int',
    'act_date_int_group_1_rt','group_1_rt','group_act_size',True)
...
df_train_gp=secondary_feature(
    df_train_gp,'char_2_y_char_6_y_char_7_y_char_9_y_rt','fill05')

#We want to deal with the entries of the groups that are left out in the
#sampling. The way we do this is to assume their act_date_int_group_1_rt are all
#in the boundary, and are all 0.5.
t_gp_intersect=np.intersect1d(
    df_train_gp['group_1'].values,t_df_train_gp_frac['group_1'].values)
...
df_train_gp=unmask_rt(
    df_train_gp,'act_date_int_group_1_rt_boundary',['act_date_int_group_1_rt'])
df_train_gp=unmask_rt(
    df_train_gp,'char_2_y_char_6_y_char_7_y_char_9_y_rt',
    ['act_date_int_group_1_rt'])
...
enc_gp=enc_gp.fit(pd.concat(
    [df_train_gp[df_categorical_col_gp],data_test[df_categorical_col_gp]]))
t_df_cat_train_gp=enc_gp.transform(df_train_gp[df_categorical_col_gp])
t_df_spr_train_gp=hstack(
    (df_train_gp[df_not_categorical_col_gp],t_df_cat_train_gp))
...
if not kaggle_output:
    df_test_gp=df_test_gp.merge(
        test_outcome,how='left',left_on='activity_id',right_on='activity_id')
    test_outcome_gp=df_test_gp['outcome_y']
df_test_gp=secondary_feature(
    df_test_gp,'char_2_y_char_6_y_char_7_y_char_9_y_rt','fill05')
df_test_gp=df_test_gp[use_col_gp]
t_df_cat_test_gp=enc_gp.transform(df_test_gp[df_categorical_col_gp])
t_df_spr_test_gp=hstack(
    (df_test_gp[df_not_categorical_col_gp],t_df_cat_test_gp))
...
param = {'max_depth':10, 'eta':eta_gp, 'silent':1,
         'objective':'binary:logistic','nthread':6,'eval_metric':'auc',
         'subsample':tree_build_subsample,'colsample_bytree':
         col_sample_tree,'min_child_weight':0,'booster':'gbtree' }
...
# df_train_wo.rename(columns={'char_5_y_char_7_y_char_32_rt' :
# 'char_5_y_char_7_y_char_32_rt_optimistic'}, inplace=True)
...
# df_train_wo=success_rate(df_train_wo, ['char_6_y','char_38'],
# t_df_train_wo_frac)
# df_train_wo=success_rate(df_train_wo, ['char_5_y','char_7_y','char_32'],
# t_df_train_wo_frac)

# df_train_wo=unmask_rt(df_train_wo, 'char_5_y_char_7_y_char_32_rt',
# ['char_6_y_char_38_rt'])
# df_train_wo=unmask_rt(df_train_wo, 'char_6_y_char_38_rt',
# ['char_5_y_char_7_y_char_32_rt'])

# df_train_wo=secondary_feature(df_train_wo,'char_6_y_char_38_rt','fill05')
# df_train_wo=secondary_feature(df_train_wo,' char_5_y_char_7_y_char_32_rt',
# 'fill05')
...
enc_wo=enc_wo.fit(pd.concat(
    [df_train_wo[df_categorical_col_wo],data_test[df_categorical_col_wo]]))
t_df_cat_train_wo=enc_wo.transform(df_train_wo[df_categorical_col_wo])
t_df_spr_train_wo=hstack((df_train_wo[df_not_categorical_col_wo],
                          t_df_cat_train_wo))
...
t_df_spr_test_wo=hstack(
    (df_test_wo[df_not_categorical_col_wo],t_df_cat_test_wo))
...
param = {'max_depth':10, 'eta':eta_wo, 'silent':1,
         'objective':'binary:logistic', 'nthread':6,'eval_metric':'auc',
         'subsample':tree_build_subsample, 'colsample_bytree':col_sample_tree,
...
if not kaggle_output:
    ...
    print(auc_11)
    print(auc_22)
    print(roc_auc_score(np.concatenate((test_outcome_gp, test_outcome_wo)),
                        np.concatenate((bst_gp.predict(dtest_gp),
                                        prediction_mod_funct(
                                            bst_wo.predict(dtest_wo),
                                            center,confidence_wo)))))
    print(
        'kaggle optimistic',
        auc_11*gp_per_k**2+2*np.sqrt(auc_11)*gp_per_k*wo_per_k\
        +auc_22*wo_per_k**2)
    print('kaggle estimate',
          auc_11*gp_per_k**2+2*0.989*gp_per_k*wo_per_k+auc_22*wo_per_k**2)
    print('kaggle lower_bound',
          auc_11*gp_per_k**2+2*auc_12*gp_per_k*wo_per_k+auc_22*wo_per_k**2)
    print('optimistic',
          auc_11*gp_per**2+2*np.sqrt(auc_11)*gp_per*wo_per+auc_22*wo_per**2)
    print('estimate',auc_11*gp_per**2+2*0.989*gp_per*wo_per+auc_22*wo_per**2)
    print('lower_bound',
          auc_11*gp_per**2+2*auc_12*gp_per*wo_per+auc_22*wo_per**2)
else:
    ...
    act_id_wo=data_test[~ data_test['group_1'].isin(
        gp_intersect)]['activity_id']
